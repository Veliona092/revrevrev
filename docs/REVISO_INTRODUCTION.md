...
5. **Version statements disagree, and CI fails because of it.** `composer.json` declares PHP `^8.2` and the Boost guidelines say 8.5, but `composer.lock` pins packages that require **PHP ≥ 8.4** — `spatie/laravel-permission` 7.2.4 (`^8.4`) and the Symfony 8 components (`clock`, `string`, `translation`, `yaml`, `css-selector`, `event-dispatcher`). The `Tests` workflow still runs the stock Laravel matrix of `[8.2, 8.3, 8.4]`, so `composer install` aborts on the 8.2 and 8.3 legs with *"Your lock file does not contain a compatible set of packages"*, and `fail-fast: true` then cancels 8.4. Fixing it means picking a floor: either raise `composer.json` to `^8.4` and trim the CI matrix, or downgrade the offending packages back to PHP 8.2-compatible releases.
