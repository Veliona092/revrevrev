...
class ChatController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('pages.chat.index', compact('classes'));
    }
    ...
    public function start(Request $request)
    {
        ...
        if (! $this->canDirectMessage($user, $target)) {
            abort(403, 'You are not allowed to message this user.');
        }

        $existingChats = Chat::query()
            ->where('kind', 'direct')
            ->whereHas('participants', fn($q) => $q->where('users.id', $user->id))
        ...
        foreach ($existingChats as $chat) {
            $participantIds = $chat->participants->pluck('id')->sort()->values();
            $expected = collect([$user->id, $target->id])->sort()->values();
            if ($participantIds->count() === 2 && $participantIds->values()->toArray() === $expected->values()->toArray()) {
                return response()->json(['success' => true, 'chat_id' => $chat->id]);
            }
        }

        $classId = $this->sharedClassId($user, $target);
