...
{
    \Illuminate\Support\Facades\Log::info('ResetPasswordNotification::toMail called for: ' . $notifiable->email);
    ...
    try {
        \Illuminate\Support\Facades\Log::info('Attempting Gmail API reset send to: ' . $notifiable->email . ' | URL: ' . $url);

        $gmailService = new GmailService();
        $gmailService->send($notifiable->email, 'Reset Your Reviso Password', $html);

        \Illuminate\Support\Facades\Log::info('Gmail API reset send completed successfully for: ' . $notifiable->email);
    } catch (\Exception $e) {
        \Illuminate\Support\Facades\Log::error('Gmail API reset send FAILED: ' . $e->getMessage() . ' | Trace: ' . $e->getTraceAsString());
    }
